...
export default async function handler(req, res) {
  ...
  if (req.method === "PUT") {
    ...
    if (!id || iva === undefined) {
      return res.status(400).json({ error: "ID de la cotización y IVA son obligatorios" });
    }

    try {
      // Actualizar el IVA en la base de datos
      const [result] = await connection.query(
        "UPDATE cotizaciones SET iva = ? WHERE id = ?",
        [iva, id]
      );

      // Verificar si la cotización fue encontrada y actualizada
